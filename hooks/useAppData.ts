...
import { Transaction, Contact, Settlement, HistoryItem, CreditBalance, ReportData, ReportPeriod } from '../types';
...
export const useAppData = () => {
  const [transactions, setTransactions] = useLocalStorage<Transaction[]>('hisab_transactions', []);
  const [contacts, setContacts] = useLocalStorage<Contact[]>('hisab_contacts', []);
  const [settlements, setSettlements] = useLocalStorage<Settlement[]>('hisab_settlements', []);
  ...
  const fullHistory = useMemo<HistoryItem[]>(() => {
    const combined = [...transactions, ...settlements];
    return combined.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  }, [transactions, settlements]);

  const creditBalances = useMemo<CreditBalance[]>(() => {
    ...
    return Array.from(balances.entries())
      .map(([contactId, balance]) => {
        ...
        if (!contact || Math.abs(balance) < 0.01) return null;
        return {
          contact,
      ...
      .filter((item): item is CreditBalance => item !== null)
  ...
  const getContactBalance = useCallback((contactId: string): number => {
  ...
  const generateReport = useCallback((period: ReportPeriod): ReportData | null => {
    const now = new Date();
